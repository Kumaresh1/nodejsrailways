...
const travelsdata = require('../DB/activitydb');
...
route.post('/new', async (req, res) => {
  ...
  let store={};
  store.activity_number=info.activity_number;
store.name=info.name;

store.price=info.price;
store.sub_name=info.sub_name;
...
route.post('/search', async (req, res) => {
  ...
  var regexc = new RegExp(data.category, "i");

  
  if(data.category!=null)
  data.category={ '$regex' : regexc}
...
console.log("search query ", data );
  ...
  bookdata.activity_number=datareq.activity_number;  
...
var outt;
let out=await travelsdata.find({"activity_number":bookdata.activity_number})
.then(result=>{
...
let i_data={
  activity_number:bookdata.activity_number,
  user_id:bookdata.user_id,
  name:bookdata.name,
  price:bookdata.price,
  nb_of_persons:datareq.nb_of_persons,
